...
lambda = 0.6 # lamba: regualarization parameter. See page 6 equation (1.3) of paper "Beyasian ART versus conjugate gradient methods in tomographic seismic imaging: An application at Mount St. Helens, Washington" 
...
# filename = paste("slowness_bart_", partitionDim, "_", resolutionDim, sep="")
...
slshigh = array(1/velocity, resHigh^3)
vecLength = resHigh^3

parts = partHigh^2

systemsHigh = GenerateAxbSplit(cube, side, cubeDim, slshigh, partHigh, resHigh, stationNum, eventStart, eventNum, flag)
idxMap = systemsHigh$map



#### parameters for multisplitting
weight = 1/parts
multiRound = 10
sptb = array(0, resHigh^3)
sptb_init = array(0, resHigh^3/parts)
sptbX = array(list(), parts)
sptbY = array(list(), parts)
bis = array(list(), parts)

for(i in 1:parts)
{
	sptbX[[i]]$s = array(0, resHigh^3/parts)
	sptbY[[i]]$s = array(0, resHigh^3/parts)
	bis[[i]]$b = array(0, systemsHigh$originRows)
}

for(i in 1:systemsHigh$originRows)
{
	bis[[1]]$b[i] = systemsHigh$originSystem[[i]]$b
}
...
for(i in 1:parts)
{
	...
	sptbY[[i]]$s = SolveBayesianART(systemsHigh$subSystems[[i]], systemsHigh$subSystems[[i]]$rayNum, resHigh, sptb_init, rho, lambda, tolerance)
# 	sptb_temp = SolveQR(systemsHigh$subSystems[[i]], systemsHigh$subSystems[[i]]$rayNum, resHigh^3/parts)
# 	sptb_temp[which(is.na(sptb_temp))] = 0
	sptbX[[i]]$s = sptbY[[i]]$s
	sptb[which(idxMap$ib == i)] = sptbX[[i]]$s
	print(paste("slowness nonzero components: ", length(which(sptbY[[i]]$s!=0))))
}
mround = 0
###########################
print(paste("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    ", mround))
tempSlness = sptb + slshigh
sError = SquareError(systemsHigh$originSystem, systemsHigh$originRows, sptb)
nResidual = NormalEqnResidual(systemsHigh$originSystem, systemsHigh$originRows, sptb)
print(paste("++++ Square Error: ", sError))
print(paste("++++ Normal Eqn Residual: ", nResidual))
...
filename = paste("slowness_bart_", partHigh, "_", resHigh, "_", mround, sep="")
write(output, file = filename, sep = "\n")
	
print(paste("error analysis at the resolution dimension: ", resHigh))
...
for(mround in 1:multiRound)
{
	#### update bi
	B1 = SystemAx(systemsHigh$subSystems[[1]], systemsHigh$subSystems[[1]]$rayNum, sptbY[[1]]$s - sptbX[[1]]$s)
	B2 = SystemAx(systemsHigh$subSystems[[2]], systemsHigh$subSystems[[2]]$rayNum, sptbY[[2]]$s - sptbX[[2]]$s)
	B3 = SystemAx(systemsHigh$subSystems[[3]], systemsHigh$subSystems[[3]]$rayNum, sptbY[[3]]$s - sptbX[[3]]$s)
	B4 = SystemAx(systemsHigh$subSystems[[4]], systemsHigh$subSystems[[4]]$rayNum, sptbY[[4]]$s - sptbX[[4]]$s)
# 	print("--------------------------------------------------------------------------------------------------")
# 	print(B1)
# 	stop()

	bis[[1]]$b = bis[[1]]$b - weight*(B2+B3+B4)
	bis[[2]]$b = bis[[2]]$b - weight*(B1+B3+B4)
	bis[[3]]$b = bis[[3]]$b - weight*(B1+B2+B4)
	bis[[4]]$b = bis[[4]]$b - weight*(B1+B2+B3)

	#### update bi in the subsystem
	for(i in 1:parts)
	{
		for(j in 1:systemsHigh$subSystems[[i]]$rayNum)
		{
			systemsHigh$subSystems[[i]][[j]]$b = bis[[i]]$b[j]
		}
	}
	...
	# 	sptb_temp = SolveQR(systemsHigh$subSystems[[i]], systemsHigh$subSystems[[i]]$rayNum, resHigh^3/parts)
	# 	sptb_temp[which(is.na(sptb_temp))] = 0
		sptbX[[i]]$s = sptbX[[i]]$s + weight*(sptbY[[i]]$s - sptbX[[i]]$s)
		sptb[which(idxMap$ib == i)] = sptbX[[i]]$s
		print(paste("slowness nonzero components: ", length(which(sptbY[[i]]$s!=0))))
	}
...
sptb_init = array(0, resHigh^3)
# tempSlness = SolveLsqr(hierarchyResult[[i]]$originSystem, slowness, hierarchyResult[[i]]$originRows)
# tempSlness = SolveQR(systemsHigh$originSystem, systemsHigh$originRows, resHigh^3)
tempSlness = SolveBayesianART(systemsHigh$originSystem, systemsHigh$originRows, resHigh, sptb_init, rho, lambda, tolerance)
# tempSlness[which(is.na(tempSlness))] = 0
tempSlness = tempSlness + slshigh

sError = SquareError(systemsHigh$originSystem, systemsHigh$originRows, tempSlness - slshigh)
nResidual = NormalEqnResidual(systemsHigh$originSystem, systemsHigh$originRows, tempSlness - slshigh)
print(paste("++++ Square Error: ", sError))
print(paste("++++ Normal Eqn Residual: ", nResidual))
...
output[1] = resHigh
output[2:(vecLength+1)] = tempSlness[1:vecLength]
filename = paste("slowness_bart_", partLow, "_", resHigh, sep="")
write(output, file = filename, sep = "\n")
